...
module TableHelper
  ...
  def tableView(tableView, willDisplayCell:cell, forRowAtIndexPath:indexPath)
    ...
    cell.on_display if cell.respond_to?(:on_display)
    if action = cell_data[:on_display]
      if action.is_a?(Proc)
  ...
  def tableView(tableView, didEndDisplayingCell:cell, forRowAtIndexPath:indexPath)
    ...
    cell_data = default_cell_style.deep_merge(cell_data)
    apply_attributes(cell, cell_data)
    cell.on_end_display if cell.respond_to?(:on_end_display)
    if action = cell_data[:on_end_display]
      if action.is_a?(Proc)
        action.call(cell_data, cell, indexPath)
      else
        self.send(action) if self.respond_to?(action)
  ...
  def tableView(tableView, accessoryButtonTappedForRowWithIndexPath:indexPath)
    ...
    if action = cell_data[:on_accessory_button_tap]
      if action.is_a?(Proc)
        action.call(cell_data, cell, indexPath)
      else
        self.send(action) if self.respond_to?(action)
    ...
    cell.on_accessory_button_tap if cell.respond_to?(:on_accessory_button_tap)
  end
  ...
  def tableView(tableView, didSelectRowAtIndexPath:indexPath)
    ...
    cell.on_select if cell.respond_to?(:on_select)
  end
  ...
  def tableView(tableView, didDeselectRowAtIndexPath:indexPath)
    ...
    cell_data = default_cell_style.deep_merge(cell_data)
    cell = tableView.cellForRowAtIndexPath(indexPath)
    if action = cell_data[:on_deselect]
      if action.is_a?(Proc)
        action.call(cell_data, cell, indexPath)
      else
        self.send(action) if self.respond_to?(action)
    ...
    cell.on_deselect if cell.respond_to?(:on_deselect)
  end
  ...
  def tableView(tableView, cellForRowAtIndexPath:indexPath)
    ...
    if cell.nil?
      ...
      apply_attributes(cell, cell_data)
      cell.on_create if cell.respond_to?(:on_create)
      if action = cell_data[:on_create]
        if action.is_a?(Proc)
          action.call(cell_data, cell, indexPath)
        else
          self.send(action) if self.respond_to?(action)
  ...
  def tableView(tableView, commitEditingStyle:editingStyle, forRowAtIndexPath:indexPath)
    cell_data = self.sections[indexPath.section][:cells][indexPath.row]
    return if cell_data[:cell]
    cell_data = default_cell_style.deep_merge(cell_data)
    case editingStyle
    when UITableViewCellEditingStyleDelete
      if action = cell_data[:on_delete]
        if action.is_a?(Proc)
          cell = tableView.cellForRowAtIndexPath(indexPath)
          p "calling the on_delete proc #{action}"
          action.call
          action.call(cell_data, cell, indexPath)
        else
          self.send(action) if self.respond_to?(action)
    ...
    when UITableViewCellEditingStyleInsert
      if action = cell_data[:on_insert]
        if action.is_a?(Proc)
          cell = tableView.cellForRowAtIndexPath(indexPath)
          p "calling the on_insert proc #{action}"
          action.call(cell_data, cell, indexPath)
        else
          self.send(action) if self.respond_to?(action)
        end
